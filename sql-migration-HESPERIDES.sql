...
-- =====================================================
-- PASO 1: PREVIEW - Ver qué se va a actualizar
-- =====================================================

-- 1.1 Ver clases programadas de Hespérides desde 2025-11-20
SELECT
  pc.id,
  pc.name,
  pc.start_date,
...
-- 1.2 Ver participantes pendientes de confirmar
SELECT
  cp.id,
  cp.class_id,
  cp.status,
...
-- 1.3 Contar registros que se van a actualizar
SELECT
...
-- =====================================================
-- PASO 2: EJECUTAR - Actualizar Hespérides
-- =====================================================
...
WHERE cp.class_id = pc.id
  AND c.id = '7b6f49ae-d496-407b-bca1-f5f1e9370610'  -- Solo Hespérides
  AND pc.start_date >= '2025-11-20'
  AND cp.status = 'active'
  AND cp.attendance_confirmed_for_date IS NULL
  AND (cp.absence_confirmed IS NULL OR cp.absence_confirmed = false);
...
-- Si necesitas revertir, usa el archivo sql-ROLLBACK-EMERGENCY.sql
