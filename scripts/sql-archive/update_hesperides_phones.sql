-- Actualizar números de teléfono para jugadores del club Hespérides Padel
...
SET phone = CASE full_name
  ...
  ELSE phone -- Mantener el teléfono actual si no hay coincidencia
END
...
-- PASO 2: Actualizar tabla STUDENT_ENROLLMENTS
...
SET phone = CASE p.full_name
  ...
  WHEN 'Valeriano Amaya villar' THEN '651447892'
  WHEN 'victor pozo' THEN '651628880'
  ELSE se.phone -- Mantener el teléfono actual si no hay coincidencia
END
FROM profiles p
...
-- RESUMEN: Mostrar cuántos registros se actualizaron
SELECT
  'PROFILES' as tabla,
  COUNT(*) as total_actualizados,
  COUNT(CASE WHEN phone IS NOT NULL AND phone != '' THEN 1 END) as con_telefono
FROM profiles
WHERE club_id = '7b6f49ae-d496-407b-bca1-f5f1e9370610'
  AND full_name IN (
    'Adrián Cazalla Salvatierra',
...
SELECT
  ...
  COUNT(*) as total_actualizados,
  COUNT(CASE WHEN se.phone IS NOT NULL AND se.phone != '' THEN 1 END) as con_telefono
