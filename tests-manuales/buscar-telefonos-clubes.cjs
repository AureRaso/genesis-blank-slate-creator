...
// Función para extraer números de teléfono españoles de un texto
function extractPhoneNumbers(text) {
  const phones = [];

  // Patrones comunes de teléfono español
  const patterns = [
    /(\+34|0034)?\s*[689]\d{2}\s*\d{2}\s*\d{2}\s*\d{2}/g,  // Móviles
    /(\+34|0034)?\s*[89]\d{2}\s*\d{2}\s*\d{2}\s*\d{2}/g,   // Fijos
    /[689]\d{8}/g,  // 9 dígitos seguidos
    /[689]\d{2}[-\s]?\d{2}[-\s]?\d{2}[-\s]?\d{2}/g  // Con separadores
  ];
  ...
  patterns.forEach(pattern => {
    ...
    if (matches) {
      matches.forEach(match => {
        ...
        if (cleaned.length === 9 && !phones.includes(cleaned)) {
          phones.push(cleaned);
        }
      });
    }
...
// Función para buscar información del club (simulada)
async function searchClubInfo(club, localidad, provincia) {
  return new Promise((resolve) => {
    // Simulamos una búsqueda
    // En un entorno real, aquí harías:
    // 1. Búsqueda en Google Custom Search API
    // 2. Web scraping de páginas amarillas
    // 3. Búsqueda en redes sociales

    console.log(`Buscando: ${club} ${localidad} ${provincia}`);

    // Simulación de delay para no saturar
    setTimeout(() => {
      resolve({
        club: club,
        provincia: provincia,
...
async function buscarTelefonos() {
  console.log(`Iniciando búsqueda de ${clubes.length} clubes...\n`);
  ...
  for (const club of clubes) {
    try {
      const resultado = await searchClubInfo(club.club, club.localidad, club.provincia);
      resultados.push(resultado);

      procesados++;
      if (procesados % 10 === 0) {
        console.log(`Progreso: ${procesados}/${clubes.length} clubes procesados`);
      }
    } catch (error) {
      console.error(`Error procesando ${club.club}:`, error.message);
      resultados.push({
        club: club.club,
  ...
  const csv = ['Club,Provincia,Localidad,Teléfono,Fuente'];
  resultados.forEach(r => {
    csv.push(`"${r.club}","${r.provincia}","${r.localidad}","${r.telefono}","${r.fuente}"`);
  });

  fs.writeFileSync('telefonos-clubes.csv', csv.join('\n'), 'utf8');

  console.log(`\n✓ Búsqueda completada!`);
  console.log(`✓ Resultados guardados en: telefonos-clubes.csv`);
  console.log(`✓ Total clubes procesados: ${resultados.length}`);

  // Estadísticas
  const encontrados = resultados.filter(r => r.telefono !== 'No encontrado' && r.telefono !== 'Error').length;
  console.log(`✓ Teléfonos encontrados: ${encontrados}`);
  console.log(`✓ No encontrados: ${resultados.length - encontrados}`);
}

// Ejecutar
buscarTelefonos().catch(console.error);
