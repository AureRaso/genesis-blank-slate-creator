...
// Función para hacer una búsqueda en DuckDuckGo HTML
...
function guardarResultados(resultados, archivo = 'telefonos-clubes.csv') {
  const csv = ['Club,Provincia,Localidad,Telefonos,Fuente,Estado'];
  resultados.forEach(r => {
    csv.push(`"${r.club}","${r.provincia}","${r.localidad}","${r.telefonos}","${r.fuente}","${r.estado}"`);
  });
...
async function buscarTelefonos() {
  console.log(`\n${'='.repeat(60)}`);
  console.log('BÚSQUEDA DE TELÉFONOS DE CLUBES DE PÁDEL');
  console.log(`${'='.repeat(60)}\n`);
  console.log(`Total de clubes a procesar: ${clubes.length}\n`);
  ...
  for (const club of clubes) {
    procesados++;
    console.log(`[${procesados}/${clubes.length}] ${club.club}`);
    ...
    try {
      ...
      // Delay para no saturar las búsquedas
  ...
  console.log(`✓ No encontrados: ${resultados.filter(r => r.estado === 'no_encontrado').length}`);
  console.log(`✗ Errores: ${resultados.filter(r => r.estado === 'error').length}`);
  console.log(`\n✓ Resultados guardados en: telefonos-clubes.csv\n`);

  // Guardar también en JSON para análisis
  fs.writeFileSync('telefonos-clubes.json', JSON.stringify(resultados, null, 2), 'utf8');
  console.log(`✓ Resultados guardados en: telefonos-clubes.json\n`);
}
